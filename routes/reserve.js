...
router.get("/filter", verifyToken, async (req, res) => {
	try {
		...
		if (req.query["data"]) {
			let date = req.query["data"].inicio.split("-");
			let day_begin = await new Date(date[0], date[1], date[2]);
			date = req.query["data"].fim.split("-");
			let day_end = await new Date(date[0], date[1], date[2]);

			reserves = await reserves.filter(reserve => reserve.date[0].day_begin <= day_begin && reserve.date[0].day_end >= day_end)

		}

		if (req.query["sala"]) {
			reserves = await reserves.filter(reserve => reserve.room.cod === req.query["sala"])

		}

		if (req.query["status"]) {
			reserves = await reserves.filter(reserve => reserve.status === req.query["status"])

		}

		if (!req.query["adminToo"]) {
			reserves = reserves.filter(reserve => reserve.user.admin === false);
		}

		reserves = reserves.filter(function (reserve, index, self) {
			return index == self.indexOf(reserve);
		});
